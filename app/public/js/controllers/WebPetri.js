	...
	*/
	...
	function startListenerForNewTransitions(){
		...
		background.on("click",function(){
			var mousePos = stage.getMousePosition();
			drawTransition(mousePos);
		});

		background.on("tap",function(){
			var touchPos = stage.getTouchPosition();
			drawTransition(touchPos);
		});
	}
		...
		*/
		...
		groups.each(function(group) {
			...
			children.each(function(shape) {
				...
				if(shape.getId().startsWith("P")){
					shape.setDrawHitFunc(function(canvas){
						var context = canvas.getContext();
						context.beginPath();
						context.arc(0, 0, this.getRadius() + 10, 0, Math.PI * 2);
						context.closePath();
				...
				}else if(shape.getId().startsWith("T")){
					shape.setDrawHitFunc(function(canvas){
						var context = canvas.getContext();
						context.beginPath();
						context.rect(-10, -10, this.getWidth() + 20, this.getHeight() + 20);
				...
				if(shape.getId().startsWith("P") || shape.getId().startsWith("T")){
					shape.on("click", function(){
						...
						if(tmpArcJs == null){
							...
							isDrawingArc = false;
							tmpArc = tmpArcJs = null;
							layer.draw();
							
							var groups = stage.get("Group");
							groups.each(function(group) {
								var children = group.getChildren();
								children.each(function(shape2) {
									shape2.off("mouseover");
								});
							});
						}
					});
		...
		groups = stage.get("Group");
		groups.each(function(group) {
			var children = group.getChildren();
			children.each(function(shape) {
				if(shape.getId().startsWith("P")){
					var groups2 = stage.get("Group");
					groups2.each(function(group2) {
						var children2 = group2.getChildren();
						children2.each(function(shape2) {
							if(shape2.getId().startsWith("T")){	
								shape2.on("mouseover", function(){
									if(isDrawingArc){
										...
										switch(direction){
											case DOWN:
												tmpArcJs.getPoints()[length].x = shape2.getX() + (T_WIDTH / 2);
												tmpArcJs.getPoints()[length].y = shape2.getY() + T_HEIGHT;
												break;
											case LEFT:
												tmpArcJs.getPoints()[length].x = shape2.getX();
												tmpArcJs.getPoints()[length].y = shape2.getY() + (T_HEIGHT / 2);
												break;
											case UP:
												tmpArcJs.getPoints()[length].x = shape2.getX() + (T_WIDTH / 2);
												tmpArcJs.getPoints()[length].y = shape2.getY();
												break;
											case RIGHT:
												tmpArcJs.getPoints()[length].x = shape2.getX() + T_WIDTH;
												tmpArcJs.getPoints()[length].y = shape2.getY() + (T_HEIGHT / 2);
												break;
										}
								...
								shape2.on("mouseover", function(){
									if(isDrawingArc){
										...
										switch(direction){
											case DOWN:
												tmpArcJs.getPoints()[length].x = shape2.getX();
												tmpArcJs.getPoints()[length].y = shape2.getY() + P_RADIUS;
												break;
											case LEFT:	
												tmpArcJs.getPoints()[length].x = shape2.getX() - P_RADIUS;
												tmpArcJs.getPoints()[length].y = shape2.getY();
												break;
											case UP:
												tmpArcJs.getPoints()[length].x = shape2.getX();
	...
	function startDragPlaces(){
		...
		groups.each(function(group) {
			...
			group.on('dragstart', function(){
				moveArcToLayer(group, arcsLayer, drawerLayer);
			});
			*/
	...
	/*
	socket.on('dragPlace', function(data){
		var place = placesLayer.get("#" + data.id)[0];
		place.setX(data.x);
		place.setY(data.y);
		placesLayer.drawScene();
	});
